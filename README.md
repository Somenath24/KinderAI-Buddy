...
**KinderAI Buddy** is an interactive, child-friendly AI web application that engages kids in safe, adaptive, and educational conversations. It features personalized chat, adaptive difficulty, memory extraction, skill tracking, face/hand detection, playful multimedia responses, and agentic self-reflection with goal adaptation.
...
- Flask (with Flask-Session for robust server-side session management)
- OpenAI API (ChatGPT, Whisper)
- ElevenLabs API (TTS)
- MediaPipe (hand detection)
- OpenCV (face detection)
- face_recognition (face recognition)
- SQLite (persistent storage for chat, profiles, reflections)
- Unsplash API (story image generation)
